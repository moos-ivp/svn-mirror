...
/*    FILE: CPAEngineThin.cpp                                    */
/*    DATE: May 12th 2005                                        */
/*    DATE: January 2016 Major revision                          */
/*    DATE: July 2017 Major revision - to "FAST-CPA"             */
/*    DATE: November 2017 Thin version from full CPAEngine       */
/*                                                               */
...
CPAEngineThin::CPAEngineThin(double cny, double cnx, double cnh,
			     double cnv, double osy, double osx)
  : CPAEngineRoot(cny, cnx, cnh, cnv, osy, osx)
{
  CPAEngineThin::reset(cny, cnx, cnh, cnv, osy, osx);
}

//----------------------------------------------------------
// Procedure: reset

void CPAEngineThin::reset(double cny, double cnx, double cnh,
			  double cnv, double osy, double osx)
{
...
double CPAEngineThin::getARange(double osh, double osv, double time) const
{
  if((osh >= 360) || (osh < 0))
    osh = angle360(osh);
...
double CPAEngineThin::getARangeRate(double osh, double osv, double time) const
{
  if((osh >= 360) || (osh < 0))
    osh = angle360(osh);
...
double CPAEngineThin::evalCPA(double osh, double osv, double ostol) const
{
  if((osh >= 360) || (osh < 0))
    osh = angle360(osh);
...
double CPAEngineThin::evalTimeCPA(double osh, double osv, double ostol) const
{
  if((osh >= 360) || (osh < 0))
    osh = angle360(osh);
...
double CPAEngineThin::evalROC(double osh, double osv) const
{
...
double CPAEngineThin::evalRangeRateOverRange(double osh, double osv,
					     double time) const
{
  return(0);
}
...
double CPAEngineThin::minMaxROC(double speed, double heading_clicks, 
...
double CPAEngineThin::bearingRate(double osh, double osv) const
{
...
double CPAEngineThin::bearingRateOld(double osCRS, double osSPD)
{
...
double CPAEngineThin::ownshipContactRelBearing(double osh) const
{
  if((osh < 0) || (osh >= 360))
    osh = angle360(osh);
...
double CPAEngineThin::smallAngle(double ang_a, double ang_b) const
{
...
bool CPAEngineThin::crossesBow(double osCRS, double osSPD) const
{
...
double CPAEngineThin::crossesBowDist(double osh, double osv) const
{
  double dist;
...
bool CPAEngineThin::crossesBowDist(double osh, double osv, double& xdist) const
{
...
bool CPAEngineThin::crossesStern(double osh, double osv) const
{
  if(m_stat_os_on_sternline)
    return(true);
...
double CPAEngineThin::crossesSternDist(double osh, double osv) const
{
  double xdist;
...
bool CPAEngineThin::crossesSternDist(double osh, double osv, double& xdist) const
{
...
bool CPAEngineThin::crossesBowOrStern(double osCRS, double osSPD) const
{
...
bool CPAEngineThin::turnsRight(double present_heading, double heading) const
{
...
bool CPAEngineThin::turnsLeft(double present_heading, double heading) const
{
  double delta = (heading - present_heading);
  if((delta < 0) || (delta >= 360))
    delta = angle360(heading - present_heading);
...
bool CPAEngineThin::passesPort(double osh, double osv) const
{
  //============================================================
...
double CPAEngineThin::passesPortDist(double osh, double osv) const
{
  double dist;
...
bool CPAEngineThin::passesPortDist(double osh, double osv, double& xdist) const
{
...
bool CPAEngineThin::passesStar(double osh, double osv) const
{
  //============================================================
  // Handle Special Cases
  //============================================================
...
double CPAEngineThin::passesStarDist(double osh, double osv) const
{
  double dist;
...
bool CPAEngineThin::passesStarDist(double osh, double osv, double& xdist) const
{
  // Until shown otherwise, xdist is -1, meaning os does not pass cn
  xdist = -1;
...
bool CPAEngineThin::passesPortOrStar(double osh, double osv) const
{
...
double CPAEngineThin::getOSSpeedInCNHeading(double osh, double osv) const
{
  if((osh < 0) || (osh >= 360))
    osh = angle360(osh);
  double v_os_cnh = m_os_cnh_cos_cache[(unsigned int)(osh)] * osv;
...
double CPAEngineThin::getOSSpeedGamma(double osh, double osv) const
{
  if((osh < 0) || (osh >= 360))
    osh = angle360(osh);
...
double CPAEngineThin::getOSSpeedEpsilon(double osh, double osv) const
{
  if((osh < 0) || (osh >= 360))
    osh = angle360(osh);
  double v_os_cnh = m_os_cnh_cos_cache[(unsigned int)(osh)] * osv;
...
double CPAEngineThin::getOSTimeGamma(double osh, double osv) const
{
...
double CPAEngineThin::getOSTimeEpsilon(double osh, double osv) const
{
  // Sanity check 1: If on the contact, return 0
...
void CPAEngineThin::setOSForeOfContact()
{
...
void CPAEngineThin::setOSAftOfContact()
{
  // First, edge case where ownship and contact are exact same position
  if(m_stat_os_on_contact) {
    m_stat_os_aft_of_cn = false;
...
void CPAEngineThin::setOSPortOfContact() 
{
  // First, edge case where ownship and contact are exact same position
  if(m_stat_os_on_contact) {
    m_stat_os_port_of_cn = false;
...
void CPAEngineThin::setOSStarboardOfContact() 
{
  // First, edge case where ownship and contact are exact same position
  if(m_stat_os_on_contact) {
    m_stat_os_star_of_cn = false;
...
void CPAEngineThin::initTrigCache()
{
...
void CPAEngineThin::initOSCNRelBngCache()
{
  vector<double> virgin_cache(360,0);
...
void CPAEngineThin::initOSCNTangentCache()
{
...
void CPAEngineThin::initOSGammaCache()
{
  //---------------------------------------------------------------------
...
void CPAEngineThin::initOSCNHCosCache()
{
...
void CPAEngineThin::initK1Cache()
{
...
void CPAEngineThin::initK2Cache()
{
  // Part 2: Create the course resolution cache for use with ownship
  vector<double> virgin_cache(360,0);
...
void CPAEngineThin::initRateCache()
{
  // Part 1: Determine the speed of contact in the direction of ownship
